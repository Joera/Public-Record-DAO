...
#[marine]
pub fn fs_put(path: &String, name: &String, file_content: Vec<u8>) -> String {

    let rpc_tmp_filepath = format!("{}{}", path, name);

    let result = fs::write(PathBuf::from(rpc_tmp_filepath.clone()), file_content);
    
    if let Err(e) = result {

        return format!("file can't be written: {}", e.to_string());
    }

    let mut s : String = "file written at ".to_owned();
    s.push_str(&rpc_tmp_filepath.as_str());

    s
}

#[marine]
pub fn fs_get(path: &String, file_name: &String) -> Vec<u8> {
